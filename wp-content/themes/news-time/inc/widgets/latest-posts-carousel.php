...
if (!class_exists('newsbiz_post_carousel')) {

  class newsbiz_post_carousel extends WP_Widget
  {
    public function __construct()
    {
      $widget_ops = array(
        'classname' => 'newsbiz_latestpost_carousel',
        'description' => __('(blog carousel style) Displays latest posts or posts from a choosen category.', 'news-time'),
        'customize_selective_refresh' => true,
      );
      parent::__construct('newsbiz-latestpost-carousel', __('&hearts; newsbiz-Post Carousel', 'news-time'), $widget_ops);
      $this->alt_option_name = 'newsbiz_latestpost_carousel';
    }
...
?>
    ...
    ?>
      ...
      <p>
        <label for="<?php echo $this->get_field_id('slidesto_show'); ?>"><?php esc_html_e('slides to show', 'news-time'); ?></label>
      ...
      <p>
        <label for="<?php echo $this->get_field_id('auto_play'); ?>"><?php esc_html_e('Auto play', 'news-time') ?></label>
        <select id="<?php echo $this->get_field_id('auto_play'); ?>" name="<?php echo $this->get_field_name('auto_play'); ?>" class="widefat">
          <option value="true" <?php if ('true' == $instance['auto_play']) echo 'selected="selected"'; ?>><?php esc_html_e('ON', 'news-time') ?></option>
          <option value="false" <?php if ('false' == $instance['auto_play']) echo 'selected="selected"'; ?>><?php esc_html_e('OFF', 'news-time') ?></option>
        </select>
      </p>
...
function newsbiz_latest_post_carousel()
{
  register_widget('newsbiz_post_carousel');
}
add_action('widgets_init', 'newsbiz_latest_post_carousel');
